...
class ESP32S31StubLoader(ESP32S31ROM):
    """Access class for ESP32-S31 stub loader, runs on top of ROM."""
...
def LoadFirmwareImage(chip, filename):
    ...
    with open(filename, 'rb') as f:
        if chip == 'esp32':
            ...
            return ESP32S31FirmwareImage(f)
        elif chip == 'esp32c3':
...
class ESP32S31FirmwareImage(ESP32FirmwareImage):
    """ESP32-S31 Firmware Image almost exactly the same as ESP32FirmwareImage"""
    ...
    def set_mmu_page_size(self, size):
        if size not in [8192, 16384, 32768, 65536]:
            raise FatalError(
                "{} bytes is not a valid ESP32-S31 page size, "
                "select from 64KB, 32KB, 16KB, 8KB.".format(size)
            )
        self.IROM_ALIGN = size
...
def elf2image(args):
    ...
    if args.chip == 'esp32':
        ...
        if args.secure_pad_v2:
            image.secure_pad = '2'
    elif args.chip == 'esp32s31':
        image = ESP32S31FirmwareImage()
        if args.secure_pad_v2:
            image.secure_pad = '2'
...
# Flasher stub code loaded from JSON files (see stubs/ dir for source & details)
ESP8266ROM.STUB_CODE = load_stub("esp8266")
ESP32ROM.STUB_CODE = load_stub("esp32")
ESP32S2ROM.STUB_CODE = load_stub("esp32s2")
ESP32S3ROM.STUB_CODE = load_stub("esp32s3")
ESP32S31ROM.STUB_CODE = load_stub("esp32s31")
ESP32C3ROM.STUB_CODE = load_stub("esp32c3")
ESP32C5ROM.STUB_CODE = load_stub("esp32c5")
